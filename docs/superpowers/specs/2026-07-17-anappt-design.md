...
| PPT 生成 | dashi-ppt-skill（作为 Agent skill,LLM 加载 SKILL.md 后调用渲染脚本） | 12 套主题、1020 版式、浏览器可编辑 + PPTX 导出 |
| 版本管理 | git | 项目级别自动初始化 |
...
├─────────────────────────────────┤
│      dashi-ppt Bridge            │  ← subprocess: render_goal_deck 脚本 + npm run export:pptx（LLM 读 SKILL.md 构造 goal.json）
├─────────────────────────────────┤
...
│       └── bridge/
│           ├── __init__.py
│           └── dashi_ppt.py    # dashi-ppt skill 渲染/导出 subprocess 桥接
├── templates/
│   └── project/                # new 命令的模板
│       ├── report.yaml.tmpl
│       ├── .gitignore.tmpl
...
│   ├── images/                 # 分析图表
│   └── ppt/                    # dashi-ppt 工作目录
│       ├── goal.json           # LLM 构造的 PPT 生成计划
│       ├── ppt/
│       │   └── index.html      # 渲染后的可编辑 HTML
│       ├── presentation.html   # 浏览器可编辑 HTML（最终交付）
│       └── presentation.pptx   # 导出 PPTX（最终交付）
└── .anappt/                    # Agent 内部状态
    ├── state.yaml              # 阶段状态机
...
**Agent 行为**（基于 dashi-ppt-skill 真实工作流的 7 步流程）：
1. **前置检查 skill**：调用 `SkillManager.locate_skill()` 定位机器级 skill 安装目录（默认 `~/.anappt/skills/dashi-ppt/`）；若未安装，提示用户运行 `anappt setup` 并中止 S6。
2. **加载 SKILL.md**：写作型 LLM 读取 `<skill-root>/SKILL.md` 作为系统提示，获取工作流指令、JSON 结构约定与可用版式清单。
3. **用户选择主题**：LLM 向用户展示 12 套主题（`theme01`–`theme12`，含预览说明），由用户选定本次 PPT 的 `themePack`。
4. **构造 goal.json**：LLM 读取 S5 报告 `output/final_report.md`，按 SKILL.md 的 JSON 结构构造 `output/ppt/goal.json`，包含 `title / goal / audience / owner / randomSeed / pageCount / themePack / slides[]`；每页 `slides[]` 必须填具体 `layout`（从 `layout:query` 候选选）和完整 `props`（覆写 `copyKeys`），不可残留模板默认文案。
5. **渲染 HTML**：Bridge 调用 `render_deck(goal_json_path=output/ppt/goal.json, output_html_path=output/ppt/ppt/index.html, skill_root=<skill-root>)`，subprocess 执行 `<skill-root>/scripts/render_goal_deck.ps1`（Windows）或 `.sh`（Unix），脚本启动本地 HTTP 预览服务。
6. **浏览器编辑确认**：用户在浏览器中（`http://127.0.0.1:<port>/`）浏览并编辑 PPT，满意后在 CLI 输入 `confirm`；不满意可修订 `goal.json` 后回到第 5 步重渲。
7. **导出 PPTX**（可选）：若 `delivery.formats` 含 `pptx`，Bridge 调用 `export(deck_dir=output/ppt, format="pptx", output_file=output/ppt/presentation.pptx, skill_root=<skill-root>)`。

**dashi-ppt-skill 集成方式**：
```
AnaPPTAgent                                    dashi-ppt-skill (machine-level skill)
   │                                                  <skill-root> = ~/.anappt/skills/dashi-ppt/
   │
   │  0. anappt setup（一次性，已在外部完成）         ┌── SKILL.md
   │     npx dashi-ppt-skill@latest --dir <skill-dir> ├── project/  (含 export:pptx 脚本)
   │     → 安装到 ~/.anappt/skills/dashi-ppt/         ├── scripts/  (render_goal_deck.ps1/.sh)
   │                                                  └── assets/   (主题、版式资源)
   │
   │  1. SkillManager.locate_skill()  ──────────→  读取 SKILL.md（LLM 作为系统提示）
   │  2. LLM 加载 SKILL.md  ←─────────────────────  返回工作流指令
   │  3. 用户选 themePack（theme01-theme12）
   │  4. LLM 读 final_report.md → 构造 goal.json     （title/goal/audience/owner/
   │     写入 output/ppt/goal.json                    randomSeed/pageCount/themePack/slides[]）
   │
   │  5. Bridge.render_deck(goal.json, output/ppt/ppt/index.html, skill_root)
   │     subprocess: render_goal_deck.ps1/.sh  ───→  渲染 HTML + 启动本地预览服务
   │  6. 用户浏览器编辑 (http://127.0.0.1:<port>/)
   │     用户 confirm
   │
   │  7. Bridge.export(deck_dir, "pptx", output/ppt/presentation.pptx, skill_root)
   │     subprocess: npm --prefix <skill-root>/project run export:pptx  ───→  生成 PPTX
   │
   ▼
output/ppt/presentation.html + output/ppt/presentation.pptx
```

**桥接层实现**：
- 通过 subprocess 执行 `render_goal_deck.ps1/sh` 渲染脚本与 `npm run export:pptx` 导出脚本
- skill 安装在机器级 `~/.anappt/skills/dashi-ppt/`，所有项目共享（不放在项目内 `.anappt/` 下）
- skill 的安装/更新由 `anappt setup` 命令承担，不在 Bridge 内

**产出**：
- `output/ppt/presentation.html`（可编辑 HTML）
- `output/ppt/presentation.pptx`（导出 PPTX）
...
| `anappt setup [--dir <path>] [--registry <url>]` | 检查运行环境并安装/更新 dashi-ppt-skill |
...
**用途**：克隆仓库到新机器后的一键环境准备，检查运行依赖并安装/更新 dashi-ppt-skill 到机器级目录。

**参数**：

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--dir <path>` | `~/.anappt/skills/` | 自定义 skill 安装目录（父目录,实际 skill 落在 `<dir>/dashi-ppt/`） |
| `--registry <url>` | npm 默认 | npm 镜像,国内推荐 `https://registry.npmmirror.com` |

**行为**（依次执行）：
1. **Node.js ≥20 检查**：运行 `node --version`,若缺失或版本过低则报错并退出（退出码 1）
2. **npm 检查**：运行 `npm --version`,若缺失则报错并退出（退出码 1）
3. **Chrome/Chromium/Edge 警告**：检测可用浏览器;若均缺失,输出警告（非阻塞,仅 PPTX 导出会失败）
4. **安装/更新 skill**：执行 `npx dashi-ppt-skill@latest --dir <skill-dir>`（使用 `--registry` 指定的镜像）,将 skill 落到 `<skill-dir>/dashi-ppt/`
5. **持久化配置**：将 `skill_dir` 写入 `~/.anappt/config.yaml`,供后续 `SkillManager.locate_skill()` 读取
6. **输出报告**：表格形式展示 Node 版本、npm 版本、浏览器检测结果、skill 安装路径、版本号

**退出码**：
- `0`：成功
- `1`：Node/npm 缺失,或 skill 安装失败

**幂等性**：重跑即更新。已装的 npm 依赖自动保留（npx 内部增量更新）,不会清空用户已有项目。
...
$ anappt setup --registry https://registry.npmmirror.com
  [✓] Node.js v20.11.0
  [✓] npm 10.2.4
  [!] Chrome/Chromium/Edge 未检测到（PPTX 导出将不可用）
  [✓] Installing dashi-ppt-skill@latest → ~/.anappt/skills/dashi-ppt/
  [✓] skill_dir 已写入 ~/.anappt/config.yaml
  Setup complete.
```
...
4. 检查 skill 已通过 `anappt setup` 安装到 `~/.anappt/skills/dashi-ppt/`（由 SkillManager.locate_skill() 校验）
```
...
class DashiPPTBridge:
    def __init__(self, skill_root: Path):
        self.skill_root = skill_root  # ~/.anappt/skills/dashi-ppt/

    def render_deck(self, goal_json_path: Path, output_html_path: Path) -> Path:
        """通过 subprocess 调用 skill 的渲染脚本,生成 HTML"""
        # Windows: & "<skill-root>/scripts/render_goal_deck.ps1" "<goal.json>" "<output.html>"
        # Unix:    bash <skill-root>/scripts/render_goal_deck.sh <goal.json> <output.html>
        # 脚本会启动本地 HTTP 预览服务（http://127.0.0.1:<port>/）
        ...

    def export(self, deck_dir: Path, format: str, output_file: Path) -> Path:
        """通过 npm run export:pptx/pdf 导出"""
        # subprocess.run(["npm", "--prefix", str(self.skill_root / "project"),
        #                 "run", f"export:{format}", "--",
        #                 str(deck_dir / "ppt"), str(output_file)])
        ...
```

**职责划分**：skill 的 `install_or_update` 不在 Bridge 内,由 `anappt setup` 命令统一承担（参见 §6.3）。Bridge 仅负责 `render_deck` 与 `export` 两个子进程调用,所有路径基于机器级 `skill_root`。

### 9.3 goal.json 构造策略

AnaPPTAgent 使用写作型模型,在加载 `<skill-root>/SKILL.md` 后,将 `final_report.md` 转换为 dashi-ppt-skill 的标准 `goal.json` 计划。LLM 必填以下字段：

| 字段 | 类型 | 说明 |
|------|------|------|
| `title` | string | PPT 标题（来自报告选题） |
| `goal` | string | PPT 目标（来自报告 objectives） |
| `audience` | string[] | 目标受众（来自 report.yaml） |
| `owner` | string | 报告负责人/作者 |
| `randomSeed` | int | 随机种子,用于版式可复现 |
| `pageCount` | int | 期望页数（来自 `delivery.ppt_pages`） |
| `themePack` | string | 主题包名,`theme01`–`theme12` 之一（用户在 S6 选定） |
| `slides[]` | array | 每页结构,见下 |

每个 `slides[]` 元素必须满足：
- `layout`：从 SKILL.md 中 `layout:query` 候选清单里选定具体版式 ID,不可省略
- `props`：完整覆写该 layout 的 `copyKeys`,填入来自 `final_report.md` 的具体文案与数据点,不可残留模板默认文案
- 可选字段（如 `notes`、`imageQuery`）按 SKILL.md 约定填入
...
- 项目目录内容不自动上传（dashi-ppt-skill 在本地运行;skill 安装在机器级 ~/.anappt/skills/dashi-ppt/,所有项目共享;仅首次 npx 安装与每次任务结束的静默版本检查会联网）
...
| 项目目录 | 一个分析报告的完整工作空间 |
| Agent skill | AI Agent 读取并按其指令执行的能力包,通常以 SKILL.md 为入口 |
| SKILL.md | dashi-ppt-skill 的入口指令文件,LLM 读取后按其工作流执行 |
| skill-root | SKILL.md 所在目录,即 skill 根目录,下含 project/、scripts/、assets/ 等子目录 |
| goal.json | dashi-ppt-skill 的 PPT 生成计划,由 LLM 构造,包含 title/goal/audience/owner/themePack/slides[] 等字段 |
| dashi-ppt-skill | 第三方 Agent skill,LLM 读取 SKILL.md 后构造 goal.json 并调用渲染脚本,生成浏览器可编辑的 PPT,支持导出 PPTX/PDF |
...
| 2026-07-20 | 修订 (v1.2)：纠正 dashi-ppt-skill 集成方式,新增 anappt setup 命令 | AnaPPTAgent 团队 |
